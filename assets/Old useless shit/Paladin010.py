...
started in the morning 10/08/2022 09:56
LETS GOOOOOOOO
10:56
headache
11:23 first script almost done, problem with character moving
Big Black Nigga Balls HD
its 00:01 11/08/2022
gonna modify the dialogue text func for the narrator
maybe put the dialogues and the scripts in one list?
its 01:34, couldnt finish my job
next time, imma fully fix the script/dialogue thing

to do list:
-write a fucking proper code
-write a function that does scripts,
-add combat
-finish the demo
...
def dialogue(text_list):
    if text_list != []:
        if dtl[i][0] != "wait func" or dtl[i][0] != "move func":
...
albert = Player(-monitor_size[0]/2, monitor_size[1]//2, 128, 128, monitor_size[0]/500, albert_img)#speed was 5
paladin = Player(monitor_size[0] * 4/5, monitor_size[1]//2, 128, 128, monitor_size[0]/500, paladin_img)#the y was 300
paladin.ll = True
paladin.lr = False
bandit_img = get_images("Bandit")
bandit1 = Player(-monitor_size[0]/2, monitor_size[1]//2, 128, 128, monitor_size[0]/400, get_images("Bandit"))
bandit2 = Player(-monitor_size[0]/2 - monitor_size[0] * 0.2, monitor_size[1]//2 + monitor_size[1] * 0.1, 128, 128, monitor_size[0]/400, get_images("Bandit"))
bandit3 = Player(-monitor_size[0]/2 - monitor_size[0] * 0.2, monitor_size[1]//2 - monitor_size[1] * 0.1, 128, 128, monitor_size[0]/400, get_images("Bandit"))
characters = [albert, paladin, bandit1, bandit2, bandit3]

dtl1 = [["Our story begins with the end of another one, The paladin, the bravest soldier of the king's army is guarding the gates at the entrance of the town, but something seems off.", "narrator"], ["move func", [characters[0], ["wr", "lr"]], 270], ["wait func", 15], ["You still here?", characters[0]], ["wait func", 45], ["You should get going friend, we have nothing to do...the town has been destroyed. You can see it by yourself.", characters[0]], ["wait func", 60], ["Look back! At the gates! Don't you see the destruction?!", characters[0]], ["I made a promise to the king, I promised that I'll guard this gates until the day I perish.", characters[1]], ["Bu-but there are no people to protect anymore! The king's de-", characters[0]], ["move func", [characters[2], characters[3], characters[4]], 200, ["wr", "lr"]]]
dtl = []
for i in range(len(dtl1)):
    if dtl1[i][0] != "wait func" or dtl1[i][0] != "move func":
...
def script_ds(wc, characters, sl):#wr, wl, wu, wd, lr, ll):
    draw_screen()
    for character in characters:
        if wc > 0:
            wc -= 1
            ...
            character.wr, character.wl, character.wu, character.wd = False, False, False, False
    return wc
        ...
        if not fcb:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE and dtl[i][0] != "wait func" or dtl[i][0] != "move func":
                        dtl.pop(0)
            ...
            if dtl[i][0] != "wait func" or dtl[i][0] != "move func":
                dialogue(dtl)
            if first_script:
                walk_counter = script_ds(walk_counter, characters[0], True, False, False, False, True, False)
                if walk_counter == 0:
                    wait_counter = wait(wait_counter)
                if wait_counter == 0:
                    dtl = dtl1
                    first_script = False
            if dtl == []:
