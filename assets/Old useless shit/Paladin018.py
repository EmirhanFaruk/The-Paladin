...
started in 25/08/2022 15:50 or smtg
gave a break at 17:33
started again at 18:23
...
bandit1 = Player(-monitor_size[0]/2, monitor_size[1]//4, 128, 128, 50, 40, 5, monitor_size[0]/200, get_images("Bandit"), "I fried my balls")
bandit2 = Player(-monitor_size[0]/2 - monitor_size[0] * 0.2, monitor_size[1]//4 - monitor_size[1] * 0.07, 128, 128, 50, 40, 5, monitor_size[0]/200, get_images("Bandit"), "Bandit on the back")
bandit3 = Player(-monitor_size[0]/2 - monitor_size[0] * 0.2, monitor_size[1]//4 + monitor_size[1] * 0.07, 128, 128, 50, 40, 5, monitor_size[0]/200, get_images("Bandit"), "I hate myself")
characters = [albert, paladin, bandit1, bandit2, bandit3]


def dialogue(text_list, characters):
    if text_list != []:
...
for i in range(len(dtl1)):
    if dtl1[i][0] != "wait func" and dtl1[i][0] != "move func" and dtl1[i][0] != "fade away":
        if dtl1[i][1] != "narrator":
            ...
            dtl1[i][0] = D_Text(monitor_size[0] * 0.05 , monitor_size[1] * 0.78, dtl1[i][0], 50, (0, 0, 0), monitor_size[0] * 0.87)


def draw_screen():
    win.blit(backgrounds[bg_stage], ((0, 0)))
    if bg_stage != 0:
        dialogue(dtl, characters)
        for character in characters:
