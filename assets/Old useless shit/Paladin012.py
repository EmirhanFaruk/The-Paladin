...
started in the morning 11/08/2022 10:06
wasted like half an hour watching vids
lets go.
its 10:29, imma give a break. i did the algorithm of the combat and some of the code of the combat
...
def combat_choice(player_and_teams):#player, player_team, enemy_team
    while
    if player in team1:
        return
...
def combat(team1, team2, turn):
    ...
    if turn > len(team1) + len(team2):
        turn = 0
    current_player = ""
    while current_player == "":#choosing the player with health more than 0
        if turn < len(team1):
            if team1[turn].health > 0:
                current_player = [turn, team1, team2]
                team1, team2 = combat_choice(current_player)
            else:
                turn += 1
                continue
        else:
            if team2[turn].health > 0:
                current_player = [turn, team2, team1]
                team2, team1 = combat_choice(current_player)
            else:
                turn += 1
                continue
