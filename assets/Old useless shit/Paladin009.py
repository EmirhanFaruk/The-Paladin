...
started in the morning 09/08/2022 11:41
the fucking dialogue thing line skip made me crazy, now imma fix it
fixed it in fucking 10 mins
lets gooo
now gotta do the start menu
did it in fucking 9 mins
its 12:00
FUCK YEAH
i actually might have to update the start menu like a flashing "press any key to continue" text and the fading black effect
...
def dialogue(text_list, person_list):
    ...
    if text_list != []:
        ...
        text_list[0].draw(win)#da_text.draw(win)
...
for i in range(len(dtl)):
    dtl[i] = D_Text(monitor_size[0] * 0.26 , monitor_size[1] * 0.78, dtl[i], 50, (0, 0, 0))

dpl = [paladin, albert, paladin, albert]


def first_dialogue_ds():
    dialogue(dtl, dpl)
    paladin.draw(win)
