...
started in 29/08/2022 10:15
there's like 2-3 days left to the university
i gotta get used to wake up at 6 am smh
now lets go make that combat ui
https://www.youtube.com/watch?v=XRxvgNQZm8E&ab_channel=Eminem-Topic
today's vibe
duh nvm didnt even have my breakfast
k its 11:21, i think i can start to work
its 14:58 rn, spent some of the time by watching vids etc, but i did the enemy combat system, the only thing that we need now is the player team combat system
its 16:18, wasted 1h on some bs
its 19:45 rn and holy shit im almost done with the demo but for some reason, the last script just wont work
...
backgrounds = [game_title, background, background, background]
bg_stage = 0
...
class Player():
    ...
    def attack(self, enemy):
        ...
        if enemy.health <= 0:
            enemy.health = 0
            pygame.mixer.find_channel().play(die_sound_effect)
...
def get_images(name, nickname = ""):
    ...
    #getting text name
    img_list.append(D_Text(monitor_size[0] * 0.12 , monitor_size[1] * 0.94, nickname, 30, (0, 0, 0), monitor_size[0] * 0.69, "middle"))#its actually too much but idc tbh it wont be seen in the game i hope
...
bandit1 = Player(-monitor_size[0]/2, monitor_size[1]//4, 128, 128, 50, 30, 5, monitor_size[0]/200, get_images("Bandit", "I fried my balls"), "I fried my balls")
bandit2 = Player(-monitor_size[0]/2 - monitor_size[0] * 0.2, monitor_size[1]//4 - monitor_size[1] * 0.07, 128, 128, 50, 30, 5, monitor_size[0]/200, get_images("Bandit", "Bandit on the back"), "Bandit on the back")
bandit3 = Player(-monitor_size[0]/2 - monitor_size[0] * 0.2, monitor_size[1]//4 + monitor_size[1] * 0.07, 128, 128, 50, 30, 5, monitor_size[0]/200, get_images("Bandit", "I hate myself"), "I hate myself")
...
for i in range(len(dtl1)):
    if dtl1[i][0] != "wait func" and dtl1[i][0] != "move func" and dtl1[i][0] != "fade away":
        if dtl1[i][1] != "narrator":
            dtl1[i][0] = D_Text(monitor_size[0] * 0.26, monitor_size[1] * 0.78, dtl1[i][0], 30, (0, 0, 0), monitor_size[0] * 0.69, "left")#0.69 is the ratio for the dialogue bubble with the head(funny reddit number) 
        else:
            dtl1[i][0] = D_Text(monitor_size[0] * 0.05, monitor_size[1] * 0.78, dtl1[i][0], 30, (0, 0, 0), monitor_size[0] * 0.87, "left")
...
decision = ["", None]
p_wait = 0
game_over_text = D_Text(monitor_size[0]/2, monitor_size[1]/4, "GAME OVER", monitor_size[0]//5, (255, 0, 0), monitor_size[0], "middle")#x, y, text, size, colour, width_limit, leftormiddle)
died = False
        ...
        if fcb:
            ...
            if in_combat and len(team1) > 0 and len(team2) > 0:
                ...
                if len(team1) == 0:
                    ...
                    game_over_text.draw(win)
                    if not died:
                        pygame.mixer.find_channel().play(game_over_sound_effect)
                        died = True
                if len(team2) == 0:
                    for event in pygame.event.get():
                        if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if combat_ending_wait <= 0:
                        ...
                        dtl = dtl2
                        characters = team1
                        if characters == ["Paladin"]:
                            dtl = dtl3
                        elif characters == ["Albert"]:
                            dtl = dtl4
    elif bg_stage == 3:#after combat
        if fcb:
