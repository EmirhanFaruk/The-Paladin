...
started in 31/08/2022
no comments i got a haircut and we ended the project with perfection
...
fart_sound = pygame.mixer.Sound(gf + "\\assets\\sounds\\echo-fart-sound-effect.mp3")

for i in range(len(sword_list)):
    sword_list[i].set_volume(0.3)
...
ending1 = pygame.image.load(gf + "\\assets\\Backgrounds\\ending1.png")
ending2 = pygame.image.load(gf + "\\assets\\Backgrounds\\ending2.png")
ending3 = pygame.image.load(gf + "\\assets\\Backgrounds\\ending3.png")
ending1 = pygame.transform.scale(ending1, (monitor_size[0], monitor_size[1]))
ending2 = pygame.transform.scale(ending2, (monitor_size[0], monitor_size[1]))
ending3 = pygame.transform.scale(ending3, (monitor_size[0], monitor_size[1]))

backgrounds = [game_intro_1, game_intro_2, game_title, background, background, background, credit1, credit2, ending1, ending2, ending3]#8, 9, 10
bg_stage = 0
for i in range(3):
...
class Player():
    ...
    def attack(self, enemy):
        self.attacking = True
        if self.name == "Bandit2":
            pygame.mixer.find_channel().play(fart_sound)
        else:
...
bandit1 = Player(-monitor_size[0]/2, monitor_size[1]//4, 128, 128, 50, 30, 5, monitor_size[0]/200, get_images("Bandit", "Bandit"), "Bandit1")
bandit2 = Player(-monitor_size[0]/2 - monitor_size[0] * 0.2, monitor_size[1]//4 - monitor_size[1] * 0.07, 128, 128, 50, 30, 5, monitor_size[0]/200, get_images("Bandit", "I fart real hard"), "Bandit2")
bandit3 = Player(-monitor_size[0]/2 - monitor_size[0] * 0.2, monitor_size[1]//4 + monitor_size[1] * 0.07, 128, 128, 50, 30, 5, monitor_size[0]/200, get_images("Bandit", "I fried my balls"), "Bandit3")
    ...
    if text_list != []:
        ...
        if not in_combat and not(bg_stage in [0, 1, 2, 6, 7]):
            ...
            for teammate in team1:
                ...
                pygame.draw.rect(win, (150, 0, 0), pygame.Rect(monitor_size[0] * 0.65, teammate.img[-1].y + (teammate.img[-1].size * 1.5), monitor_size[0] * 0.30, teammate.img[-1].size))
                pygame.draw.rect(win, (0, 200, 100), pygame.Rect(monitor_size[0] * 0.65, teammate.img[-1].y + (teammate.img[-1].size * 1.5), hp_per, teammate.img[-1].size))
            ...
            for teammate in team2:
                ...
                hp_per = (monitor_size[0] * 0.30) * (teammate.health / teammate.max_health)
                #pygame.draw.rect(surface, color, pygame.Rect(30, 30, 60, 60))
                pygame.draw.rect(win, (150, 0, 0), pygame.Rect(monitor_size[0] * 0.04, teammate.img[-1].y + (teammate.img[-1].size * 1.5), monitor_size[0] * 0.30, teammate.img[-1].size))
                pygame.draw.rect(win, (0, 200, 100), pygame.Rect(monitor_size[0] * 0.04, teammate.img[-1].y + (teammate.img[-1].size * 1.5), hp_per, teammate.img[-1].size))
...
dtl2 = [["wait func", 30], ["move func", [["Albert", ["lr"]]], 10], ["Phew! That was close! Well, as I was saying-", "Albert"], ["I\'m tired of killing bandits here, I want to know from where and why they are coming.", "Paladin"], ["Hmmm... These bandits must be bountyhunters that the Slime King has hired.", "Albert"], ["What are we waiting for then? Let\'s go find that guy!!!", "Paladin"], ["move func", [["Paladin", ["wl", "ll"]], ["Albert", ["wl", "ll"]]], 450], ["...and so, our brave heroes went inside the forest to end the Slime King\'s life!", "narrator"], ["fade away"]]
dtl3 = [["wait func", 5], ["Albert, no...", "Paladin"], ["I WILL MAKE THESE PEOPLE PAY FOR WHAT THEY DID!", "Paladin"], ["move func", [["Paladin", ["wl"]]], 450], ["Raged by his best friend\'s death, Paladin ran towards the forest, hoping to find where these bandits came from while not knowing the adventures he\'s getting into...", "narrator"], ["fade away"]]
dtl4 = [["wait func", 5], ["move func", [["Albert", ["lr"]]], 10], ["Paladin...", "Albert"], ["wait func", 30], ["Well, now that the Paladin is dead, I ain\'t got things to do here anymore. Here I go Valdimore!", "Albert"], ["move func", [["Albert", ["wl", "ll"]]], 450], ["fade away"]]
dtl = []
...
def draw_screen():
    ...
    if not(bg_stage in [0, 1, 2, 8, 9, 10]):
...
def fade_gb(width, height, win, backgrounds, bgs, bgsw): 
    ...
    while alpha <= 255:
        ...
        win.blit(backgrounds[bg_stage], ((0, 0)))
        if not(bgs in [0, 1, 2, 6, 7, 8, 9, 10]):
            dialogue(dtl, characters, in_combat)
            draw_screen()
        win.blit(fade, (0,0))
        fps_counter()
...
died = False
combat_ending_wait = 30
combat_ending_wait2 = 30
died_fr = False
...
while running:
    ...
    draw_screen()
    nmp = (mp[0] - 2, mp[1] - 2)
    if died and not(bg_stage in [0, 1, 2, 6, 7, 8, 9, 10]):
    ...
    if bg_stage == 2:
        ...
        if will_fade:
            ...
            get_class("Albert", characters).img[-1].y = monitor_size[1] * 0.76 + monitor_size[1] * 0.07
            ...
            get_class("Bandit1", characters).img[-1].y = monitor_size[1] * 0.76 + monitor_size[1] * 0.07
            ...
            get_class("Bandit3", characters).img[-1].y = monitor_size[1] * 0.76 + monitor_size[1] * 0.14
        ...
        if not fcb and not will_fade:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    speak_blip.stop()
                    if dtl != []:
                        if event.key == pygame.K_SPACE and dtl[0][0] != "wait func" and dtl[0][0] != "move func" and dtl[0][0] != "fade away":
                            dtl.pop(0)
                            sound_check = False

            if len(dtl) > 0 and not(sound_check):
                if dtl[0][0] != "wait func" and dtl[0][0] != "move func" and dtl[0][0] != "fade away":
                    speak_blip.play()
                    sound_check = True
            ...
            if in_combat and len(team1) > 0 and len(team2) > 0:
                ...
                if len(team2) == 0:
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.KEYDOWN:
                            combat_ending_wait = 0
                    combat_ending_wait2 = wait(combat_ending_wait2)
                    if combat_ending_wait2 <= 0:
                        in_combat = False
                        characters = team1
        ...
        if will_fade and ending_wait <= 0:
            if died_fr:
                will_fade, bg_stage = fade_gb(monitor_size[0], monitor_size[1], win, backgrounds, bg_stage, 10)
                game_over_sound_effect.play(-1)
            else:
                if ending_number == 2:
                    will_fade, bg_stage = fade_gb(monitor_size[0], monitor_size[1], win, backgrounds, bg_stage, 9)
                elif ending_number == 1:
                    will_fade, bg_stage = fade_gb(monitor_size[0], monitor_size[1], win, backgrounds, bg_stage, 8)
                else:
                    running = False
                    break
    ...
    elif bg_stage == 8:#ending1
        if fcb:
            fcb, bg_stage = fade_cb(monitor_size[0], monitor_size[1], win, backgrounds, bg_stage, 8)
            will_fade = True
    ...
    elif bg_stage == 9:#ending2
        if fcb:
            fcb, bg_stage = fade_cb(monitor_size[0], monitor_size[1], win, backgrounds, bg_stage, 9)
            will_fade = True
    ...
    elif bg_stage == 10:#ending3
        if fcb:
            fcb, bg_stage = fade_cb(monitor_size[0], monitor_size[1], win, backgrounds, bg_stage, 10)
            will_fade = True
    ...
    if not(bg_stage in [0, 1, 2, 6, 7, 8, 9, 10]):
        if md:
